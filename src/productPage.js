...
$('.slider-bottom').slick({
  ...
  focusOnSelect: true,
  nextArrow:
    '<img class="product-slider-arrow arrow-right" src="product-page/arrowproduct.svg">',
  prevArrow:
    '<img class="product-slider-arrow arrow-left" src="product-page/arrowproduct.svg">',
})
$('.product-reviews__wrapper').slick({
  ...
  nextArrow:
    '<img class="product-reviews-slider-arrow right-arrow" src="./icons/topslider/right-arrow.svg">',
  prevArrow:
    '<img class="product-reviews-slider-arrow left-arrow" src="./icons/topslider/left-arrow.svg">',
})
...
const showMoreProductDescBtn = document.querySelector(
...
showMoreProductDescBtn.addEventListener('click', () => {
  ...
  showMoreProductDescBtn.textContent = moreTextProductDesc.classList.contains(
    'more-text-hidden'
  )
    ? 'Смотреть все'
    : 'Скрыть'
...
reviewTexts.forEach((text) => {
  if (text.textContent.length > 210) {
    ...
    text.nextElementSibling.addEventListener('click', () => {
      ...
      text.nextElementSibling.textContent = text.classList.contains(
        'hide-full-review'
      )
        ? 'Показать весь текст'
        : 'Скрыть'
...
const anotherMaterialBtn = document.querySelector('.choose-material'),
  anotherMaterialBackGround = document.querySelector(
...
document
  .querySelector('.choose-material-top__title img')
  .addEventListener('click', () => {
    document.documentElement.style.overflow = 'auto'
    anotherMaterialBackGround.classList.remove(
...
document.addEventListener('click', (e) => {
  if (
    anotherMaterialBackGround.classList.contains(
      'choose-sidebar-background-visible'
    ) &&
    e.target === anotherMaterialBackGround
  ) {
    document.documentElement.style.overflow = 'auto'
    anotherMaterialBackGround.classList.remove(
      'choose-sidebar-background-visible'
...
document.querySelectorAll('.select-img').forEach((item) => {
  item.addEventListener('click', () => {
    document
      .querySelector('.material-selected')
      .classList.remove('material-selected')
...
document.querySelectorAll('.product-info__choise-color img').forEach((img) => {
  img.addEventListener('click', () => {
    document
      .querySelector('.product-info__choise-color img.selected')
      .classList.remove('selected')
    img.classList.add('selected')
  })
})
setInterval(() => {
  console.log('asfas'), 3000
})
